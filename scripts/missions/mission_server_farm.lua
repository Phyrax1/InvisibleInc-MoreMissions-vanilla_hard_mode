-- hard mode: spawn 4 2.0 daemons instead of 3 normal ones
-- TODO: check if all 2.0 daemons actually work correctly when installed on devices/stacked with others or themselves
---------------------------------------------------------------------------------------------------------------
...
local USE_TERM = {action = "abilityAction", fn = function(sim, ownerID, userID, abilityIdx, ...)
...
local function useServerTerminal(script, sim, mission)
    ...
    if #possibleUnits > 0 then
        ...
        script:queue(
            {type = "showIncognitaWarning", txt = STRINGS.UI.WARNING_NEW_DAEMON,
             vo = "SpySociety/VoiceOver/Incognita/Pickups/Warning_New_Daemon"}
        )
        script:waitFrames(.75 * cdefs.SECONDS)

        for k = 1, 4 do -- increased to 4
            if #possibleUnits > 0 then
